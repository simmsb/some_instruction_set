...
ERROR:
  printf("[ERROR] failed to decode instruction, [RIP] = %" PRIu16
         " instruction = %" PRIu16 " n args = %d\n",
...
void halt(OPERATION_I) { cpu->flags.halt = false; }

void nop(OPERATION_I) {
  // yo it's a no op
}

void ret(OPERATION_I) { cpu_setreg(cpu, rip, cpu_popstack(cpu)); }
...
void jmp(OPERATION_I) { cpu_setreg(cpu, rip, arg1); }

void psh(OPERATION_I) { cpu_pushstack(cpu, cpu_getloc(cpu, arg1)); }
...
void jeq(OPERATION_I) {
  if (cpu->flags.zero)
    cpu_setreg(cpu, rip, cpu_getloc(cpu, arg1));
}
...
void ptc(OPERATION_I) { printf("%c", (char)cpu_getloc(cpu, arg1)); }
...
void mov(OPERATION_I) { cpu_setloc(cpu, arg1, cpu_getloc(cpu, arg2)); }
...
void flc(OPERATION_I) { // convert arg1 and arg2 into float and store in float
                        // reg aaa
...
void clf(OPERATION_I) { // convert float register aaa into two ints and store in
                        // arg1 and arg2
...
void stf(OPERATION_I) { // store upper 2 bytes of float in arg1, lower 2 bytes
                        // in arg2 (no cast)
  float value = cpu->fregs[aaa];
  uint32_t c = *(uint32_t *)&value;
...
void ldf(
    OPERATION_I) { // load a float from memory into float register aaa (no cast)
  uint16_t lower = cpu_getloc(cpu, arg2);
  uint16_t upper = cpu_getloc(cpu, arg1);
