  ...
  while (cpu->flags.halt) {
    ...
    switch (instr) {
      ...
      case 0x4003: {
        ...
        if (cpu->flags.zero) cpu_setreg(cpu, rip, loc);
        break;
      } // jeq
      ...
      case 0x4005: {
        uint16_t loc = cpu_getloc(cpu, cpu->memory[cpu->regs[rip]++]);
        if (cpu->flags.sign) cpu_setreg(cpu, rip, loc);
        break;
      } // jle


      case 0x4006: {
        if (!cpu->flags.sign) cpu_setreg(cpu, rip, cpu_getloc(cpu, cpu->memory[cpu->regs[rip]++]));
        break;
      } // jme
